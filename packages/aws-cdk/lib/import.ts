...
export class ResourceImporter {
   ...
   */
  public async importResourcesFromMap(importMap: ImportMap, options: DeployStackOptions) {
    const resourcesToImport: ResourcesToImport = await this.makeResourcesToImport(importMap);
    const updatedTemplate = await this.currentTemplateWithAdditions(importMap.importResources);

    return this.importResources(updatedTemplate, resourcesToImport, options);
  }
   ...
   * @param options Options to pass to CloudFormation deploy operation
   */
  public async importResourcesFromMigrate(resourcesToImport: ResourcesToImport, options: DeployStackOptions) {
    const updatedTemplate = this.removeNonImportResources();

    return this.importResources(updatedTemplate, resourcesToImport, options);
  }

  private async importResources(overrideTemplate: any, resourcesToImport: ResourcesToImport, options: DeployStackOptions) {
    try {
